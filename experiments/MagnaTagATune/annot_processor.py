...
def reduce_to_N_tags(filename, base_dir, n_tops=config.NUM_TAGS, merge=True):
    ...
    Return : 
        new_filename : path to the improved_implementation processed csv file with reduced tags
    '''
    if merge:
    ...
    print (df.shape)
    # save improved_implementation csv file
...
def split_data(filename, base_dir, ratio=0.2):
    ''' Split into train/val/test and saves each set to a improved_implementation file
    Args: 
        filename : path to the MTT annotation csv file 
